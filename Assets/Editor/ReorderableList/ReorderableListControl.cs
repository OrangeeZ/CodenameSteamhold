...
namespace Rotorz.ReorderableList {
	...
	public class ReorderableListControl {
		...
		private void PrepareState(int controlID, IReorderableListAdaptor adaptor) {
			...
			_visibleRect = GuiUtility.VisibleRect();
		...
		private bool DoRemoveButton(Rect position, bool visible) {
			...
			return GuiUtility.IconButton(position, visible, iconNormal, iconActive, ItemButtonStyle);
		}
			...
			try {
				...
				if (isRepainting && isVisible) {
					...
					if (isDraggable) {
                        ...
                        GuiUtility.DrawTexture(texturePosition, ReorderableListResources.GetTexture(ReorderableListTexture.GrabHandle));
					}
					...
					if (!_tracking || itemIndex != s_AnchorIndex) {
						if (itemIndex != 0 || HorizontalLineAtStart) {
                            ...
                            GuiUtility.Separator(horizontalLinePosition, HorizontalLineColor);
						}
					}
				}
		...
		private void DrawFloatingListItem(IReorderableListAdaptor adaptor, float targetSlotPosition) {
			if (Event.current.type == EventType.Repaint) {
                ...
                GuiUtility.Separator(targetPosition, HorizontalLineColor);
                ...
                GuiUtility.DrawTexture(targetPosition, EditorGUIUtility.whiteTexture);
                ...
                GuiUtility.DrawTexture(s_DragItemPosition, EditorGUIUtility.whiteTexture);
...
//*/
		...
		private void AutoScrollTowardsMouse() {
			...
			Rect visiblePosition = GuiUtility.VisibleRect();
			Vector2 mousePosition = Event.current.mousePosition;
		...
		private void CheckForAutoFocusControl() {
			...
			if (s_AutoFocusControlID == _controlID) {
                ...
                GuiUtility.FocusTextInControl("AutoFocus_" + _controlID + "_" + s_AutoFocusIndex);
				s_AutoFocusIndex = -1;
			}
		}
		...
		private void DrawFooterControls(Rect position, IReorderableListAdaptor adaptor) {
			if (HasFooterControls) {
				...
				if (HasAddButton) {
					...
					if (GuiUtility.IconButton(buttonPosition, true, iconNormal, iconActive, FooterButtonStyle)) {
						// Append item to list.
						GUIUtility.keyboardControl = 0;
						AddItem(adaptor);
					}
				...
				if (HasAddMenuButton) {
					// Draw add menu drop-down button.
					if (GuiUtility.IconButton(menuButtonPosition, true, menuIconNormal, menuIconActive, FooterButtonStyle)) {
						GUIUtility.keyboardControl = 0;
