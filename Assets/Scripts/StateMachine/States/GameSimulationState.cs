...
namespace Assets.Scripts.StateMachine.States
{
    public class GameSimulationState : State<GameState>
    {
        ...
        private BaseWorld InitializeSimulationWorld()
        {
            ...
            var playerWorld = CreatePlayerWorld();

            //create player
            var player = CreatePlayer(playerWorld);
            CreateWorldEvents(playerWorld, player, _unitFactory);
            //init parent world
            ...
            InitializeOnGuiDrawer(playerWorld, player, _unitFactory);
            return world;
        }

        private BaseWorld CreatePlayerWorld()
        {
            ...
            _unitFactory.SetWorld(playerWorld);
            // create dummy stockpile
            var stockpile = _unitFactory.CreateBuilding(BuildingType.StockpileBlock);
            stockpile.SetPosition(Vector3.zero);
            playerWorld.Stockpile.AddStockpileBlock(stockpile as StockpileBlock);

            //todo test start limit
            playerWorld.PopulationLimit = 8;
            var stopckpile = playerWorld.Stockpile.GetClosestStockpileBlock(Vector3.zero);
            stopckpile.ChangeResource(ResourceType.Bread, 100);
